...
async function fetchOrganizerEvents(token, organizerId, orgName, status, pageSize = 50) {
...
export async function loadEventsPayload() {
  ...
  try {
    ...
    for (const org of ORGANIZERS) {
      ...
      collected.push(...await fetchOrganizerEvents(token, org.id, org.name, 'ended', 15))
    }
